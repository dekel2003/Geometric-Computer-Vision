...
T45(T45>=1e7) = Inf;
A=colorDistanceMap(T45);
imshow(A);

%% -----Q4-------%
P=load('pool'); %load maze
P=double(P.n); %double for our c mex func
P0=[500;400];
Pf=[1;1];
src = P0;
t0=[0];
pool_display='Silent';
figure(6),
[pool_T] = run_fmm(P, src, t0,pool_display);
%figure(7),
%imshow(P);
minPath = shortestPath(pool_T,Pf); %geodesic path to the source points.
title('Shortest path');